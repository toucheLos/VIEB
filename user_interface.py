...
class RunPipelineView(QWidget):
    ...
    def _build(self):
        ...
        for stage in STAGES:
            ...
            v.addWidget(row)
        v.addWidget(self._build_diagnostics_panel())
        v.addStretch()
    ...
    def update_diagnostics(self, data: dict):
        """Populate the diagnostics panel from loaded clustering diagnostics."""
        ...
        for warning in warnings:
            ...
            if level == "error":
                ...
                color, icon = "#1565c0", "i"
            lbl = QLabel(f"  {icon}  {warning.get('message', '')}")
            lbl.setWordWrap(True)
            lbl.setStyleSheet(f"color:{color};font-size:11px;padding:2px 0;")
            if warning.get("action"):
                lbl.setToolTip(warning["action"])
            self._diag_warnings_lay.addWidget(lbl)

        if not _MPL:
            return

        if self._diag_occ_canvas is not None and occ is not None and not occ.empty:
            ...
            if not state_rows.empty:
                ...
                try:
                    from _utils import _state_colors as _diag_state_colors
                    colors = _diag_state_colors(max(len(states), 1))
                except Exception:
                    colors = plt.cm.tab20(np.linspace(0, 1, max(len(states), 1)))
                ax.barh(
                    range(len(states)),
                    fracs,
                    color=[colors[int(s) % len(colors)] for s in states],
                    alpha=0.85,
                )
        ...
        if self._diag_umap_canvas is not None and umap_path.exists():
            ...
            try:
                ...
                if not noise.empty:
                    ax.scatter(noise["umap_1"], noise["umap_2"], c="#CCCCCC", s=1, alpha=0.3, rasterized=True)
                if not valid.empty:
                    ax.scatter(valid["umap_1"], valid["umap_2"], c=valid["label"], cmap="tab20", s=1, alpha=0.5, rasterized=True)
                ax.set_xticks([])
                ax.set_yticks([])
                ax.set_title("UMAP Embedding (sampled)", fontsize=10, fontweight="bold", loc="left")
            except Exception:
                ax.text(0.5, 0.5, "Error loading UMAP sample", ha="center", va="center", transform=ax.transAxes, color="#999")
            canvas.fig.tight_layout()
            canvas.draw()

        if self._diag_conf_canvas is not None and umap_path.exists():
            canvas = self._diag_conf_canvas
            canvas.fig.clf()
            ax = canvas.fig.add_subplot(111)
            try:
                udf = pd.read_csv(umap_path)
                ax.hist(udf["prob"].values, bins=50, color="#4E79A7", alpha=0.8, edgecolor="white", linewidth=0.3)
    ...
    def _regen_diagnostics(self):
        if self._worker and self._worker.isRunning():
            QMessageBox.information(self, "Pipeline busy", "Wait for the current pipeline step to finish.")
            return
        self._start_worker_cmd(
            [sys.executable, "compare.py", "--diagnostics"],
            "Regenerating diagnostics...",
        )
