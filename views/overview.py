...
class OverviewView(QWidget):
    ...
    def __init__(self):
        ...
        self._banner.setStyleSheet(
            "QFrame#gsBanner{background:#EEF4FB;border:1px solid #C8DAEC;border-radius:6px;}"
        )
        ...
        hdr_lbl.setStyleSheet("color:#4E79A7;background:transparent;border:none;")
        ...
        self._show_dominant_chk = QCheckBox("Highlight dominant state")
        self._show_dominant_chk.setChecked(True)
        self._show_dominant_chk.stateChanged.connect(lambda _: self._render_states_bar())
        states_hdr.addWidget(self._show_dominant_chk)
        lay.addLayout(states_hdr)
        if _MPL:
            self._canvas_states = MplCanvas(figsize=(10, 3.5))
            self._canvas_states.setMinimumHeight(200)
        ...
        self._nd_toggle_btn.setStyleSheet(
            "font-size:12px;color:#4E79A7;text-align:left;border:none;background:transparent;"
        )
    ...
    def _active_state_cols(self):
        ...
        if summary is None:
            return []
        # Use all state columns present in the dataframe — don't rely on cluster_info count
        cols = sorted(
            [c for c in summary.columns if c.startswith("state_") and c.endswith("_frac")],
    ...
    def _render_states_bar(self):
        ...
        # Use ALL state columns present in the dataframe — don't rely on cluster_info
        state_cols = sorted(
            [c for c in summary.columns if c.startswith("state_") and c.endswith("_frac")],
            key=lambda c: int(c.split("_")[1]),
        )
        if not state_cols:
            ax.text(0.5, 0.5, "No state columns found in summary_table.",
                    ha="center", va="center", transform=ax.transAxes,
                    fontsize=10, color="#9B9B9B")
            ax.axis("off")
            canvas.draw()
        ...
        dom_id = int(means.idxmax().split("_")[1]) if not means.empty else -1
        highlight = self._show_dominant_chk.isChecked()
        colors = [
            "#E07B39" if (sid == dom_id and highlight) else "#4E79A7"
            for sid in state_ids
        ]

        ax.set_facecolor("#FFFFFF")
        canvas.fig.patch.set_facecolor("#FFFFFF")

        if n > 20:
            # Horizontal bars are cleaner when there are many states
            # Resize canvas height to accommodate
            h = max(3.5, n * 0.22)
            canvas.fig.set_size_inches(10, h)
            canvas.setMinimumHeight(int(h * 80))

            bars = ax.barh(state_ids, values, color=colors, height=0.7)
            ax.set_yticks(state_ids)
            ax.set_yticklabels([str(s) for s in state_ids], fontsize=7)
            ax.invert_yaxis()
            ax.set_xlabel("Mean fraction of session", fontsize=9, color="#6B6B6B")
            ax.set_ylabel("State ID", fontsize=9, color="#6B6B6B")
            if highlight and dom_id >= 0:
                idx = state_ids.index(dom_id) if dom_id in state_ids else -1
                if idx >= 0:
                    ax.annotate(
                        f"  dominant (S{dom_id})",
                        xy=(values[idx], dom_id),
                        fontsize=7, color="#E07B39", va="center",
                    )
        else:
            bars = ax.bar(state_ids, values, color=colors, width=0.7)
            ax.set_xticks(state_ids)
            ax.set_xticklabels(
                [str(s) for s in state_ids],
                fontsize=7 if n > 10 else 8,
            )
            ax.set_xlabel("State ID", fontsize=9, color="#6B6B6B")
            ax.set_ylabel("Mean fraction of session", fontsize=9, color="#6B6B6B")
            if highlight and dom_id >= 0 and dom_id in state_ids:
                dom_val = values[state_ids.index(dom_id)]
                ax.annotate(
                    f"S{dom_id}",
                    xy=(dom_id, dom_val), xytext=(0, 4),
        ...
        ax.set_title(
            f"State occupancy — all sessions ({n} states)",
            fontsize=10, color="#1A1A1A", pad=8,
        )
    ...
    def _render_disc(self):
        ...
        ax.axhline(0.0, color="#D4D4D4", linewidth=1.0, linestyle="-")
        ax.axhline(0.2, color="#E5E5E5", linewidth=1.0, linestyle="--")
        ax.text(0.99, 0.03, "chance", color="#9B9B9B", fontsize=7,
                ha="right", va="bottom", transform=ax.transAxes)
        ax.text(0.99, 0.24, "criterion", color="#B0B0B0",
                fontsize=7, ha="right", va="bottom", transform=ax.transAxes)
        ax.set_xlabel("Day", fontsize=9, color="#6B6B6B")
        ax.set_ylabel("Discrimination Ratio", fontsize=9, color="#6B6B6B")
    ...
    def _render_occupancy(self):
        ...
        ax.set_xlabel("State ID", fontsize=8, color="#6B6B6B")
        ax.set_ylabel("Mean fraction", fontsize=8, color="#6B6B6B")
        ax.set_title("State occupancy by cohort", fontsize=9, color="#1A1A1A")
        if state_ids:
            ax.set_xticks(state_ids)
            ax.set_xticklabels([str(s) for s in state_ids], fontsize=6 if len(state_ids) > 20 else 7)
        ax.tick_params(colors="#6B6B6B", labelsize=7)
        if groups:
            ax.legend(fontsize=7, framealpha=0.9, edgecolor="#E5E5E5")
        ax.set_facecolor("#FFFFFF")
        canvas.fig.patch.set_facecolor("#FFFFFF")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
    ...
    def _render_fear_enriched(self):
        ...
        ax.bar(state_ids, vals, color=bar_colors, alpha=0.85)
        ax.axhline(0, color="#D4D4D4", linewidth=0.8)
        ax.set_xlabel("State ID", fontsize=8, color="#6B6B6B")
        ax.set_ylabel("Context A − B", fontsize=8, color="#6B6B6B")
        ax.set_title("Fear-enriched states (A vs B)", fontsize=9, color="#1A1A1A")
        if state_ids:
            ax.set_xticks(state_ids)
            ax.set_xticklabels([str(s) for s in state_ids], fontsize=6 if len(state_ids) > 20 else 7)
        ax.tick_params(colors="#6B6B6B", labelsize=7)
        ax.annotate("red = more in fear context", xy=(0.02, 0.96),
                    xycoords="axes fraction", fontsize=7, color="#C0392B", va="top")
        ax.set_facecolor("#FFFFFF")
        canvas.fig.patch.set_facecolor("#FFFFFF")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
