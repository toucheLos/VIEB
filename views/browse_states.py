...
from _widgets import VideoPlayer, MplCanvas
...
class BrowseStatesView(QWidget):
    ...
    def _refresh_clips(self):
        ...
        if not clip_dir.exists() or not list(clip_dir.glob("*.mp4")):
            ...
            self._clip_page = 0
            self._clear_clip_grid()
            no_lbl = QLabel("No clips available. Click 'Generate Clips' to create them.")
    ...
    def _render_clip_page(self):
        ...
        for i, fp in enumerate(page_files):
            ...
            name_lbl = QLabel(fp.stem[:22])
            name_lbl.setAlignment(Qt.AlignCenter)
            name_lbl.setStyleSheet("font-size:9px;color:#555;")
            cell = QWidget()
            cl = QVBoxLayout(cell)
