...
python characterize.py         # Behavioral state profiles + t-SNE → results/characterize/
```
...
| Module | Class | Role |
|--------|-------|------|
| `feature_extraction.py` | `PoseFeatureExtractor` | Pose → kinematic/spatial/postural features `(T, 51)` |
| `preprocessing.py` | `BehaviorPreprocessor` | Standardize + PCA; has separate `fit()` / `transform()` |
| `clustering.py` | `BehaviorClusterer` | K-Means/GMM; auto-tunes from k=4–15 using silhouette |
...
`compare.py` fits **one shared preprocessor + clusterer** on all 1.28M frames pooled together, then predicts labels for each video using that shared model, then applies HMM Viterbi smoothing to correct single-frame jitter. This is the correct path for cross-condition comparisons. The shared models are saved in `results/shared/` (preprocessor as `.pkl` via `BehaviorPreprocessor.save()`, clusterer via `joblib`).
...
results/
  ...
  characterize/          # From characterize.py
    cluster_tsne.png
    feature_heatmap.png
    context_contrast.png
```
